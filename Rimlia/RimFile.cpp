...
int CPathString::OpenDirBrowser(HWND hWnd, LPTSTR p_cSetStr, LPTSTR p_cRootStr, LPTSTR p_cCaptionStr, LPTSTR result, UINT p_uiFlags){
	...
	stBInfo.pszDisplayName	= chReturn;
	stBInfo.lpszTitle		= p_cCaptionStr;
	stBInfo.ulFlags			= p_uiFlags;
	stBInfo.lpfn			= (BFFCALLBACK)BrowseCallbackProc;
	stBInfo.lParam			= (LPARAM)p_cSetStr;
