...
{
    // 呼び出し側でメモリ割り当てを行えるようにカラーバッファを作成しバインドする
#if 0
...
{
#if 0
	// フレームバッファオブジェクトを作成し、カラーバッファをアタッチする（関連づける）
...
void RenderingEngine2::Render() const
{
	...
	tt.SetXYZ(-1.0f,-1.0f,1.0f,1.0f);
	tt.SetDiffuse(1.0f,1.0f,1.0f,1.0f);
...
void RenderingEngine2::UpdateAnimation(float timeStep)
{
    ...
    // 角度が0≦m_currentAngle＜360の範囲内に納まるようにする
    if (m_currentAngle >= 360)
        m_currentAngle -= 360;
    else if (m_currentAngle < 0)
        m_currentAngle += 360;
    
    // 回転方向が変わったということは目標の角度を超えたということ
